...
import { ArrowRight, Check, Package, Zap, Clock, BarChart3, Smartphone, Wifi, Loader2 } from 'lucide-react';
import { Button } from '../components/ui/button';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '../components/ui/card';
import { Badge } from '../components/ui/badge';
import { usePageContent } from '../hooks/usePageContent';

const OrderSystemComplete = () => {
  const { content, loading } = usePageContent('order-system');
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div className="space-y-6">
              <Badge className="bg-blue-500 text-white border-none">
                {content?.hero?.subtitle || 'Système de Commande'}
              </Badge>
              <h1 className="text-5xl md:text-6xl font-bold leading-tight">
                {content?.hero?.title || 'Système de Commande Centralisé'}
              </h1>
              <p className="text-xl text-blue-100">
                {content?.hero?.description || 'Gérez toutes vos commandes sur un seul écran'}
              </p>
            ...
            <div className="relative">
              <img
                src={content?.hero?.image || 'https://images.unsplash.com/photo-1555396273-367ea4eb4db5?w=800&q=80'}
              ...
              {content.benefits.map((benefit, index) => (
                <div key={index} className={`grid lg:grid-cols-2 gap-12 items-center ${index % 2 === 1 ? 'lg:flex-row-reverse' : ''}`}>
                  <div className={index % 2 === 1 ? 'lg:order-2' : ''}>
                    <img
                      src={benefit.image || 'https://images.unsplash.com/photo-1555396273-367ea4eb4db5?w=800&q=80'}
                      alt={benefit.title}
      ...
      <section className="py-20 bg-gradient-to-br from-blue-600 to-indigo-900 text-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-xl text-blue-100 mb-8">
            Rejoignez des centaines de restaurants qui nous font confiance
          </p>
...
export default OrderSystemComplete;
